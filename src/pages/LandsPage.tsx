...
import { Property, Land, LandFilters } from "@/types";
import { landsData, sortProperties } from "@/store/property.store";
...
const LandsPage: React.FC = () => {
  ...
  const sizeOptions = [
    ...
    { value: "0-500", label: "Up to 500 sq.yd." },
    { value: "500-1000", label: "500 - 1,000 sq.yd." },
    { value: "1000-5000", label: "1,000 - 5,000 sq.yd." },
    { value: "5000+", label: "5,000+ sq.yd." },
  ];
  ...
  const applyFilters = () => {
    let filtered = [...landsData];

    if (filters.location) {
      filtered = filtered.filter((land) => land.city === filters.location);
    }
    ...
    if (filters.size) {
      ...
      if (filters.size.endsWith("+")) {
        filtered = filtered.filter((land) => land.sizeValue >= min);
      } else {
        filtered = filtered.filter(
          (land) => land.sizeValue >= min && land.sizeValue <= max,
        );
      }
    }
    ...
    if (filters.priceMin) {
      ...
      filtered = filtered.filter((land) => land.price >= min);
    }

    if (filters.priceMax) {
      const max = parseInt(filters.priceMax.replace(/[^0-9]/g, "")) || Infinity;
      filtered = filtered.filter((land) => land.price <= max);
    }
  ...
  const filteredLands = applyFilters();
      ...
      <section className="py-20 bg-gradient-to-br from-[#0f1a2c] via-[#1a3a5f] to-[#0f1a2c]">
        <div className="container mx-auto px-4 text-center">
          ...
          <p className="text-white/80 max-w-2xl mx-auto animate-slide-in-right">
            Find the perfect plot for your dream home or investment
          </p>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4">
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  <i className="fas fa-expand text-purple-600 mr-2"></i>
                  Size
                </label>
                <select
                  value={filters.size}
